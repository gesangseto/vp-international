...
class Ajax_data extends Base_controller
{
    ...
    public function get_all_task_by_order_number()
    {
        ...
        $arr = [];
        if (isset($_POST['order_number'])) {
            $query =  'SELECT f.`id`, f.`order_number`, f.`shipping_name`, f.`consignee`, f.`vessel`, f.`flight`, f.`shipper`, f.`container_no`, f.`party`, f.`mbl_no`, f.`hbl_no`, 
            ...
            FROM list_cash_advanced AS c LEFT JOIN detail_cash_advanced AS d ON c.`id` = d.`cash_advance_id`) AS e) AS g
