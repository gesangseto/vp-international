...
class Actual_cash_advanced extends Base_controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('transaction/actual_cash_advanced/index', $data);
    ...
    public function create()
    {
        ...
        $this->load->view('transaction/actual_cash_advanced/create', $data);
        $this->load->view('templates/Footer');
    }
    ...
    public function read()
    {
        ...
        $this->load->view('transaction/actual_cash_advanced/read', $data);
        $this->load->view('templates/Footer');
    }
        ...
        if (!empty($_GET['id'])) {
            ...
            if ($data['response']['statusCode'] == 200) {
                ...
                foreach (@$_POST['task_id'] as $row) {
                    ...
                    $value_inp[$n]['actual_idr'] = $_POST['actual_idr'][$n];
                    $value_inp[$n]['actual_usd'] = $_POST['actual_usd'][$n];
                    $value_inp[$n]['remark'] = $_POST['remark'][$n];
                    $n = $n + 1;
                }
                // echo json_encode($value_inp);
                // die;
                $data['response'] = $this->_Cash_advance->_update_detail_ca_batch($value_inp);
            }
            $this->session->set_flashdata('response', $data['response']);
            redirect('transaction/actual_cash_advanced/update?id=' . $id);
        }
        $this->load->view('transaction/actual_cash_advanced/update', $data);
        $this->load->view('templates/Footer');
    }
